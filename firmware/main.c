...
/* Capstone fault injection: 0 = normal immediate ISR yield,
 * 1 = notification is still delivered but the task may wait until
 * the next scheduler opportunity. This demonstrates graceful degradation. */
#ifndef FAULT_MODE
#define FAULT_MODE 0
#endif
...
static const char *TAG = "Space app3";
...
static volatile uint32_t ground_cmd_count;
static volatile uint32_t sem_compare_count;
...
static void IRAM_ATTR button_isr(void *arg)
{
     ...
     * In FAULT_MODE the event is still delivered, but immediate scheduling is
     * intentionally skipped so the latency increase can be demonstrated. */
#if !FAULT_MODE
...
#define C_LEN 1024                      /* bytes; raise toward 49152 to lengthen */
...
static void metrics_task(void *arg)
{
    for (;;) {
        ESP_LOGI(TAG,
                 "[wcet] A=%llu us B=%llu us C=%llu us D=%llu us | hb=%lu/%lu/%lu/%lu",
...
static void start_background_load(void)
{
    ...
    xTaskCreatePinnedToCore(metrics_task, "metrics", 3072, NULL, 1, NULL, PRO_CPU_NUM);
}
#endif /* WITH_LOAD */
...
void app_main(void)
{
    ...
    ESP_LOGI(TAG, "==== App 3 [Space Ground Command] starting — ISR + bottom-half ====");
...
#if FAULT_MODE
    ESP_LOGW(TAG, "FAULT MODE: immediate ISR yield disabled; commands remain functional but may be delayed");
#else
    ESP_LOGI(TAG, "Fault mode: OFF — normal immediate ISR yield");
#endif
